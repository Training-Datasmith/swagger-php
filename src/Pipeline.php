...
class Pipeline
{
    ...
    {
        if (!$pipe && !$matcher) {
            throw new Open_Api_Exception('pipe or callable must not be empty');
        }
        // allow matching on class name if $pipe in a string
        if (is_string($pipe) && !$matcher) {
            $pipe_class = $pipe;
            $matcher = static fn($pipe): bool => !$pipe instanceof $pipe_class;
        }
        if ($matcher) {
            $tmp = [];
    ...
    public function insert(callable $pipe, $matcher): Pipeline
    {
        ...
        if (null === $index || $index < 0 || $index > count($this->pipes)) {
            throw new Open_Api_Exception('Matcher result out of range');
        }
